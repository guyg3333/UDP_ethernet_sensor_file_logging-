...
void EtherCard::sendUdpBGR (const char *data, uint8_t datalen, uint16_t sport,
                         const uint8_t *dip, uint16_t dport ,const uint8_t *mac ) {
    udpPrepare(sport, dip, dport);
    ...
    if (datalen>220)
        datalen = 220;
    memcpy(gPB + UDP_DATA_P, data, datalen);
	copyMac(gPB + ETH_DST_MAC, mac); // GUY
    udpTransmit(datalen);
}
